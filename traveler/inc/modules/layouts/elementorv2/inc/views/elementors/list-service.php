            ...
            case 'st_cars':
                if (st_check_service_available('st_cars')) {
					...
					if ( is_singular( 'location' ) ) {
						global $wpdb;
						$location_id = get_the_ID();
						$sql = "SELECT post_id FROM {$wpdb->prefix}st_location_relationships WHERE 1=1 AND location_from IN ({$location_id}) AND post_type IN ('st_cars')";
						$res = $wpdb->get_results($sql, ARRAY_A);
						$res = array_map ( function($re) {
							return $re['post_id'];
						}, $res );
