    ...
    if (!in_array($user_role, array('partner', 'administrator'))) {
        ...
        <div class="container">
            <h1 class="author-page-title">
                <?php
                echo __('Campground Information', 'traveler');
                ?>
            </h1>
                                            ...
                                            <ul class="author-list-info">
                                                <?php if ( isset( $user_meta['st_is_check_show_info'] ) && $user_meta['st_is_check_show_info'] == 'on' ): ?>
                                                    <?php if ( (in_array('all', $list_info) || in_array('email', $list_info)) ) { ?>
                                                        <li>
                                                            <?php
// Assuming $current_user_upage->user_email contains the user's email address
$email = $current_user_upage->user_email;
$maxLength = 10; // Maximum display length of the email address

// Check if the email length exceeds the maximum display length
if (strlen($email) > $maxLength) {
    // Shorten the email and append an ellipsis
    $displayEmail = substr($email, 0, $maxLength - 3) . '...';
} else {
    // Email is short enough to display in full
    $displayEmail = $email;
}

// Display the email. The full email is used in the mailto link, while the shortened or full email (based on length) is displayed to the user.
echo '<i class="fa fa-envelope input-icon"></i> <strong>' . __( 'Email: ', 'traveler' ) . '</strong> <a href="mailto:' . htmlspecialchars($email) . '" title="' . htmlspecialchars($email) . '">' . htmlspecialchars($displayEmail) . '</a>';
?>
                                                        </li>
                                                    <?php } ?>
