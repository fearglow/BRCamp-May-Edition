...
if (!function_exists('st_email_booking_thumbnail')) {
    function st_email_booking_thumbnail()
    {
        ...
        if ($order_id) {
            ...
            if ($image) {
                return '<a href="' . get_the_permalink($item_id) . '" target="_blank"><img alt="thumbnail" src="' . $image . '" data style="display: block; height: 108px; width: 80px;"></a>';
            }
        }
        return '';
    }
...
/**
 * Extended function to display start time or specific instructions based on post type in booking email.
 *
 * @since 2.0.0
 * Updated to include check for 'st_booking_post_type' and conditionally display messages,
 * including a formatted message for start time.
 */
if (!function_exists('st_email_booking_start_time')) {
    function st_email_booking_start_time()
    {
        global $order_id;
        if ($order_id) {
            $booking_post_type = get_post_meta($order_id, 'st_booking_post_type', true);
            if ($booking_post_type == 'st_rental') {
                return "Check in after <strong>1pm</strong>, Check out at <strong>12pm</strong>";
            } else {
                $starttime_value = get_post_meta($order_id, 'starttime', true);
                if ($starttime_value != '') {
                    return "Starting at: " . $starttime_value;
                } else {
                    return "Starting at: _";
                }
            }
        }
        return ''; 
